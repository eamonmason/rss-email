"""Lambda function to check status of podcast Anthropic Message Batch."""
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=W0613
    """
    Check status of podcast Message Batch.
    ...
    try:
        ...
        logger.info(
            "Podcast batch %s status: %s, succeeded: %d, errored: %d",
            batch_id,
        ...
        logger.error("Error checking podcast batch status: %s", e, exc_info=True)
        raise
